...
def sequence_to_Tm(sequence):
     ...
     dH     = TUPLE_ENTHALPY['INIT'] + sum([TUPLE_ENTHALPY[sequence[i:i+2]] for i in range(len(sequence)-1)])  + end_AT*TUPLE_ENTHALPY['PENL']
     dS     = TUPLE_ENTROPY['INIT']  + sum([TUPLE_ENTROPY[sequence[i:i+2]]  for i in range(len(sequence)-1)])  + end_AT*TUPLE_ENTROPY['PENL']

     #Equation from Santalucia paper (https://doi.org/10.1146/annurev.biophys.32.110601.141800 - DOI:10.1146/annurev.biophys.32.110601.141800)
     Tm_1MNaCl = 1.0*dH/(R*np.log(STAP_CONC-0.5*SCAF_CONC)+dS/1000) - 273.15

     #Fraction of G/C
     fGC    = 1.0*sum([(c == 'G' or c == 'C') for c in sequence])/len(sequence)

     #Equation 16 from salt correction paper: Richard Owczarzy et al, Biochemistry, 2008 (http://pubs.acs.org/doi/abs/10.1021/bi702363u - DOI:10.1021/bi702363u)
